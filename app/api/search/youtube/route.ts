...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const item of allResults) {
      ...
      // Try all known channelId locations
      if (!channelId) {
        channelId = (
          r.channelId
          || r.channel_id
      ...
      // Collect video-type results as fallback
    ...
    // STEP 4 — Fetch channel videos (by channelId, by extracted handle, or by query-as-handle)
    if (channelId || handle) {
      lookup = 'found'
    ...
    // If channelId/handle from search failed, try query directly as a handle
